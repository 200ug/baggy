...
A new configuration (stored to `~/.config/wsftp.conf`) can be created like this:

```
wsftp init -key <privkey_path> -remote <user>@<hostname>:<port>:<remote_storage_root>
```

After the initial config creation running `wsftp sync` in a directory will begin the sync process (whether or not the directory has been synced to the remote before). Notably the tool (intentionally) utilizes a flat remote directory structure, which means there's a risk of mixing different directories if they're named the same (e.g. `path1/dir` and `path2/dir`).

---

###### Mirrors: [Codeberg](https://codeberg.org/2ug/wsftp) / [Github](https://github.com/200ug/wsftp)
